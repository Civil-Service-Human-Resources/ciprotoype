...
exports.addNunjucksFilters = function (env) {
  let coreFilters = require('./core_filters.js')(env)
  let customFilters = require('../app/filters.js')(env)
  let filters = Object.assign(coreFilters, customFilters)
...
exports.addCheckedFunction = function (env) {
  env.addGlobal('checked', function (name, value) {
    ...
    let storedValue = this.ctx.data[name]
...
exports.getLatestRelease = function () {
  ...
  if (releaseUrl !== null) {
    ...
    let gitHubUrl = 'https://api.github.com/repos/alphagov/govuk-prototype-kit-private-beta/releases/latest'
    try {
      console.log('Getting latest release from GitHub')

      let res = request('GET', gitHubUrl, options)
      let data = JSON.parse(res.getBody('utf8'))
      let zipballUrl = data['zipball_url']
      let releaseVersion = zipballUrl.split('/').pop()
      let urlStart = 'https://github.com/alphagov/govuk-prototype-kit-private-beta/archive/'
      let urlEnd = '.zip'
      let zipUrl = urlStart + releaseVersion + urlEnd
...
exports.matchMdRoutes = function (req, res) {
  let docsPath = '/../docs/documentation/'
  if (fs.existsSync(path.join(__dirname, docsPath, req.params[0] + '.md'), 'utf8')) {
    let doc = fs.readFileSync(path.join(__dirname, docsPath, req.params[0] + '.md'), 'utf8')
...
let storeData = function (input, store) {
  for (let i in input) {
    ...
    if (Array.isArray(val)) {
      let index = val.indexOf('_unchecked')
      if (index !== -1) {
        val.splice(index, 1)
      }
    }
...
exports.getFormInputs = (query) => {
  let formInputs = false
  if (query.inputs) {
    formInputs = JSON.parse(query.inputs)
  }
  return formInputs
}

exports.getError = (query, id, message) => {
  let result
  if (query.inputs) {
    const formInputs = JSON.parse(query.inputs)
    if (formInputs[id] === '') {
      result = message
    } else {
      result = false
...
exports.isGovEmail = (email) => {
  return !!email.includes('gov.uk')
}
